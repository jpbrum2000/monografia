...
def grid_search(n_classes_conhecidas,NA,T_first,T_last):
	...
	for T in np.linspace(T_first,T_last,10):
		...
		for FOLD in range(2):
			...
			confusion_matrix = knn_dr_cv.confusion_matrix(math.ceil(n_classes_conhecidas/2), T, *divide_training_test(FOLD,*divide_know_unknow(math.ceil(n_classes_conhecidas/2),treino_labels,treino)))
...
def knn(T,n_classes_conhecidas):
	confusion_matrix = knn_dr_cv.confusion_matrix(n_classes_conhecidas,T,*divide_training_test(0,*divide_know_unknow(n_classes_conhecidas,labels,objetos)))
...
f = open('testes\knn_dr_cv\knn_result_'+str(n_classes), 'a')
