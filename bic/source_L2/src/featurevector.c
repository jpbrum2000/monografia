...
FeatureVector1DInt *CreateFeatureVector1DInt(int n)
{
  FeatureVector1DInt *fv=NULL;
  
  fv = (FeatureVector1DInt *) calloc(1,sizeof(FeatureVector1DInt));
  if (fv != NULL) {
    fv->X = AllocIntArray(n);
    fv->n = n;
  } else {
    Error(MSG1,"CreateFeatureVector1DInt");
  }
  return(fv);
}
...
void DestroyFeatureVector1DInt(FeatureVector1DInt **fv)
{
  FeatureVector1DInt *aux;
...
FeatureVector1DInt *ReadFileFeatureVector1DInt(char *filename)
{
  FeatureVector1DInt *fv = NULL;
  int i, nbins;
  int x;
  ...
  fscanf(fp, "%d\n", &nbins);
  fv = CreateFeatureVector1DInt(nbins);
  for (i=0; i<nbins; i++){
    fscanf(fp, "%d", &x);
    fv->X[i] = x;
  }
...
void WriteStreamFeatureVector1DInt(FeatureVector1DInt *fv,FILE *fp)
{
  int i;
...
void WriteFileFeatureVector1DInt(FeatureVector1DInt *fv,char *filename)
{
  FILE *fp;
  ...
  WriteStreamFeatureVector1DInt(fv, fp);
  fclose(fp);
}
