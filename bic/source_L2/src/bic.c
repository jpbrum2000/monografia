...
/*
  int i;
  printf("max=npixels=%d\n", npixels);
  printf("lowhistogram:\n");
  for (i=0; i<SIZE; i++) {
    printf("%lu ", vf->lowH[i]);
  }
  printf("\n");
  printf("highhistogram:\n");
  for (i=0; i<SIZE; i++) {
    printf("%lu ", vf->highH[i]);
  }
  printf("\n");*/
...
VisualFeature *ExtractVisualFeatures(CImage *cimg, Image *mask)
{
  VisualFeature *vf;
  Property *p;
  int npixels;
...
FeatureVector1DInt *BIC(CImage *cimg, Image *mask)
{
  ...
  FeatureVector1DInt *fvector = NULL;
  //CompressedVisualFeature *cvf;
  VisualFeature *vf;
  int i;

  //cvf = ExtractCompressedVisualFeatures(cimg, mask);
  vf = ExtractVisualFeatures(cimg, mask);

  fvector = CreateFeatureVector1DInt(2*SIZE);
  for (i=0; i<SIZE; i++){
    fvector->X[i] = vf->lowH[i];
    fvector->X[i+SIZE] = vf->highH[i];
  }

//   printf("fvector size --> %d\n", fvector->n);
//   printf("lowhistogram:\n");
//   for (i=0; i<SIZE; i++) {
//     printf("%d ", fvector->X[i]);
//   }
//   printf("\n");
//   printf("highhistogram:\n");
//   for (i=0; i<SIZE; i++) {
//     printf("%d ", fvector->X[SIZE+i]);
//   }
//   printf("\n");
